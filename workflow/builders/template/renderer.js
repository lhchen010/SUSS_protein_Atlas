...
var domainFiltered=DOMAIN_FAMILIES.slice(),domainNetwork=null,domainNetworkNodes=null,domainNetworkEdges=null,domainSegmentNetwork=null,domainViewer=null,curDomain=null,curDomainSegment=null,domainSuperpose=false,domainSelected={},domainColorMode="domain";
...
function renderDomainUnassigned(){var box=document.getElementById("unassigned"),query=searchNorm((document.getElementById("searchinput")||{}).value||""),rows=DOMAIN_UNASSIGNED.filter(function(item){return!query||searchNorm([item.acc,item.parent_family,item.label,item.p2rank_score,item.fpocket_score,Object.keys(item.expression||{}).join(" ")].join(" ")).indexOf(query)>=0;});box.innerHTML='<div class="singleton-head"><div><h2>No D-family match</h2><div class="hint">Proteins without a retained local structural-domain family under this run’s thresholds. This does not prove that the protein lacks domains.</div></div></div><div class="singleton-table-wrap"><table class="singleton-table"><thead><tr><th>Protein</th><th>Parent F family</th><th>Annotation</th><th>P2Rank</th><th>fpocket</th><th>pLDDT / length</th></tr></thead><tbody>'+rows.map(function(item){return'<tr onclick="openProtein(\''+String(item.acc).replace(/'/g,"\\'")+'\')"><td><b>'+esc(item.acc)+'</b></td><td>'+esc(item.parent_family==="singleton"?"protein singleton":item.parent_family)+'</td><td>'+esc(item.label)+'</td><td>'+fnum(item.p2rank_score,3)+(item.p2rank_probability==null?"":'<br><span class="hint">prob '+fnum(item.p2rank_probability,3)+'</span>')+'</td><td>'+fnum(item.fpocket_score,3)+'</td><td>'+fnum(item.plddt,1)+'<br><span class="hint">'+(item.length==null?"\u2013":item.length+" aa")+'</span></td></tr>';}).join("")+'</tbody></table></div>';var status=document.getElementById("searchstatus");if(status)status.textContent=rows.length+" unassigned protein"+(rows.length===1?"":"s");}
...
function buildDomainStructurePane(id,query){
 var members=domainMembers(id),edges=domainEdges(id),wb=domainWorkbench(id),matched=domainMatchingMembers(id,query||""),initial=(matched[0]||members.find(function(m){return m.segment_id===wb.hub;})||members[0]);
...
function updateDomainSelectionUi(){if(!curDomain)return;var selected=domainSelectedMembers(curDomain);document.querySelectorAll("[data-domain-check]").forEach(function(box){box.checked=!!domainSelected[box.getAttribute("data-domain-check")];});var superBtn=document.getElementById("domainSuperBtn");if(superBtn){superBtn.textContent="Superpose selected ("+selected.length+")";superBtn.disabled=selected.length<2;superBtn.className=domainSuperpose?"on":"";}document.querySelectorAll("[id^=dcm-]").forEach(function(button){button.className=button.id==="dcm-"+domainColorMode?"on":"";});var shown=domainSuperpose?selected:[DOMAIN_MEMBERS.find(function(x){return x.segment_id===curDomainSegment;})],legend=document.getElementById("domainLegend");if(legend)legend.innerHTML=shown.filter(Boolean).map(function(m){return'<span><i class="swatch" style="background:'+domainSegmentColor(m)+'"></i>'+esc(m.acc+" "+m.start+"–"+m.end)+(m.segment_id===domainWorkbench(m.domain_family).hub?" · hub":"")+'</span>';}).join("")+'<span><i class="swatch" style="background:#c7cdd0"></i>complete parent context</span>';}
...
function renderDomainStructure(m){var el=document.getElementById("domain3d");if(!el)return;var selected=domainSuperpose?domainSelectedMembers(m.domain_family):[m],missing=selected.filter(function(seg){return !domainPdb(seg)&&BACKEND.enabled&&!structureFetchFailed[seg.acc];});if(missing.length){el.innerHTML='<p class="hint" style="padding:12px">Loading selected full proteins…</p>';var pending=missing.length;missing.forEach(function(seg){fetchStructure(seg.acc,function(){pending--;if(!pending)renderDomainStructure(m);});});return;}domainViewer=$3Dmol.createViewer(el,{backgroundColor:"white"});var wb=domainWorkbench(m.domain_family),modelIndex=0;selected.forEach(function(seg){var pdb=domainPdb(seg),tr=domainSuperpose?(wb.transforms||{})[seg.segment_id]:{rotation:[[1,0,0],[0,1,0],[0,0,1]],translation:[0,0,0]};if(!pdb||!tr)return;var values=domainColorMode==="esm"?(seg.esm_values||{}):(domainColorMode==="cons"&&seg.segment_id===wb.hub?(wb.hub_structural_conservation||{}):{}),modelText=Object.keys(values).length?pdbWithValues(pdb,values,0):pdb;if(domainSuperpose)modelText=transformDomainPdb(modelText,tr);domainViewer.addModel(modelText,"pdb");domainViewer.setStyle({model:modelIndex},{cartoon:{color:"#c7cdd0",opacity:.22}});var domainStyle={color:domainSegmentColor(seg),opacity:.96};if(domainColorMode==="esm"&&Object.keys(values).length){var vals=Object.keys(values).map(function(k){return Number(values[k]);}),mn=Math.min.apply(null,vals),mx=Math.max.apply(null,vals);domainStyle={colorscheme:{prop:"b",gradient:"rwb",min:mx,max:mn},opacity:.98};}else if(domainColorMode==="cons"&&Object.keys(values).length){domainStyle={colorscheme:{prop:"b",gradient:"rwb",min:1,max:0},opacity:.98};}domainViewer.setStyle({model:modelIndex,resi:domainResidues(seg)},{cartoon:domainStyle});var pocket=domainColorMode==="p2rank"?((seg.p2rank||{}).domain_lining_residues||[]):domainColorMode==="fpocket"?((seg.fpocket||{}).domain_lining_residues||[]):[];if(pocket.length)domainViewer.setStyle({model:modelIndex,resi:pocket},{stick:{color:"#c83b35",radius:.22},sphere:{color:"#c83b35",scale:.2}});modelIndex++;});if(!modelIndex){el.innerHTML='<p class="hint" style="padding:12px">Structure unavailable.</p>';return;}fitDomainSegments();updateDomainSelectionUi();}
function buildDomainTreesPane(id){var wb=domainWorkbench(id),foldtrees=wb.foldtree_tree_svgs||{},structural=Object.keys(foldtrees).map(function(metric){return'<section class="tree-block"><h3>FoldTree structural relationship · '+esc(metric)+'</h3><img src="'+foldtrees[metric]+'"><div class="hint">Cropped domain structures; structural relationship evidence, not a sequence phylogeny.</div></section>';}).join(""),guide=wb.foldmason_guide_tree_svg?'<section class="tree-block"><h3>FoldMason structural guide tree</h3><img src="'+wb.foldmason_guide_tree_svg+'"><div class="hint">Guide tree used during multiple-structure alignment; kept separate from FoldTree.</div></section>':"",sequence=(wb.sequence_subgroups||[]).filter(function(group){return group.tree_svg;}).map(function(group){return'<section class="tree-block"><h3>Sequence relationship · '+esc(group.id)+'</h3><img src="'+group.tree_svg+'"><div class="hint">MAFFT amino-acid MSA → FastTree WAG for a BLAST-supported sequence subgroup only.</div></section>';}).join("");document.getElementById("dp1").innerHTML=(structural||'<p class="hint">FoldTree was not available or fewer than three domain structures passed.</p>')+guide+(sequence||'<p class="hint">No sequence-homologous subgroup was large enough for a sequence tree.</p>');}
...
function buildDomainSequencePane(id){var wb=domainWorkbench(id);document.getElementById("dp3").innerHTML='<div class="hint">Local Foldseek structure links define the D family. MAFFT is restricted to BLAST-supported sequence subgroups; FoldMason covers the complete structural family.</div>'+domainMsaBlock("MAFFT sequence MSA",wb.sequence_msa||{})+domainMsaBlock("FoldMason structural MSA · amino acids",wb.structural_msa||{})+domainMsaBlock("FoldMason structural MSA · 3Di",wb.three_di_msa||{})+'<div class="domain-actions"><button onclick="dlDomainSeqs(\''+esc(id)+'\')">All domain sequences</button><button onclick="dlDomainParentSeqs(\''+esc(id)+'\')">All parent sequences</button><button onclick="dlDomainMsa(\''+esc(id)+'\')">MAFFT MSA</button><button onclick="dlDomainAlignment(\''+esc(id)+'\',\'aa\')">FoldMason AA MSA</button><button onclick="dlDomainAlignment(\''+esc(id)+'\',\'3di\')">FoldMason 3Di MSA</button></div>';}
function buildDomainConservationPane(id){var wb=domainWorkbench(id),rows=domainMembers(id).map(function(m){var p2=m.p2rank||{},fp=m.fpocket||{},esm=Object.keys(m.esm_values||{}).length;return'<tr><td><b>'+esc(m.segment_id)+'</b></td><td>'+((p2.domain_lining_residues||[]).join(", ")||"none")+'</td><td>'+fnum(p2.top_score,3)+' / '+fnum(p2.top_probability,3)+'</td><td>'+((fp.domain_lining_residues||[]).join(", ")||"none")+'</td><td>'+fnum(fp.top_score,3)+'</td><td>'+esm+'</td></tr>';}).join("");document.getElementById("dp4").innerHTML='<h3>Structural conservation and parent-protein evidence</h3><div class="hint">FoldMason structural conservation is calculated on cropped domains. P2Rank and fpocket are calculated for every complete parent protein. ESM is representative-level by default; available values are mapped to domain coordinates. Cropped structures are not used for pocket prediction.</div><div class="domain-summary"><div class="domain-stat"><b>'+Object.keys(wb.hub_structural_conservation||{}).length+'</b>hub residues with FoldMason lDDT</div><div class="domain-stat"><b>'+domainMembers(id).filter(function(m){return(m.pocket_residues||[]).length;}).length+'</b>segments overlapping a pocket</div><div class="domain-stat"><b>'+domainMembers(id).filter(function(m){return Object.keys(m.esm_values||{}).length;}).length+'</b>segments with ESM values</div></div><div class="domain-edge-table"><table><thead><tr><th>Segment</th><th>P2Rank domain residues</th><th>score / probability</th><th>fpocket domain residues</th><th>score</th><th>ESM sites</th></tr></thead><tbody>'+rows+'</tbody></table></div>';}
...
function setAtlasMode(mode){
 if(mode==="domains"&&ANALYSIS_SCOPE==="full")return;if(mode==="clusters"&&ANALYSIS_SCOPE==="domain")return;
 atlasMode=mode;var single=mode==="singletons",domains=mode==="domains",unassigned=mode==="unassigned";document.getElementById("net").classList.toggle("mode-hidden",single||domains||unassigned);document.getElementById("singletons").classList.toggle("mode-hidden",!single);document.getElementById("domains").classList.toggle("mode-hidden",!domains);document.getElementById("unassigned").classList.toggle("mode-hidden",!unassigned);document.getElementById("modeclusters").className=(!single&&!domains&&!unassigned)?"on":"";document.getElementById("modedomains").className=domains?"on":"";document.getElementById("modeunassigned").className=unassigned?"on":"";document.getElementById("modesingletons").className=single?"on":"";
 var input=document.getElementById("searchinput");if(input)input.placeholder=single?"Search protein singleton":(domains?"Search D family, protein, or segment":(unassigned?"Search proteins without a D-family match":"Search full-length families"));
...
var clearSearchButton=document.getElementById("clearsearch");if(clearSearchButton)clearSearchButton.addEventListener("click",clearAtlasSearch);document.getElementById("modesingletons").textContent="Protein singletons ("+SINGLETONS.length+")";document.getElementById("modedomains").textContent="Domain families ("+DOMAIN_FAMILIES.length+")";document.getElementById("modeunassigned").textContent="No D-family match ("+DOMAIN_UNASSIGNED.length+")";if(ANALYSIS_SCOPE==="full"){document.getElementById("modedomains").style.display="none";document.getElementById("modeunassigned").style.display="none";}if(ANALYSIS_SCOPE==="domain"){document.getElementById("modeclusters").style.display="none";document.getElementById("modesingletons").style.display="none";setAtlasMode("domains");}else applyNetworkSearch("");
...
function buildStructPane(id){
 ...
 var hasStructural=hasReference(id+"_struct"),hasEvolutionary=hasReference(id+"_cons");
...
function initViewer(){var el=document.getElementById("v3d");if(!el)return;var singleton=PAY[curFam]&&PAY[curFam].kind==="singleton",member=(PAY[curFam].members||[])[0],preferred=singleton?"quality":(hasReference(curFam+"_struct")?"structcons":(hasReference(curFam+"_cons")?"evolcons":"quality")),key=preferred==="structcons"?curFam+"_struct":(preferred==="evolcons"?curFam+"_cons":"");if(BACKEND.enabled&&key&&!REFPDB[key]&&!referenceFetchFailed[key]){el.innerHTML='<p class="hint" style="padding:12px">Loading reference structure…</p>';fetchReference(key,function(){initViewer();});return;}if(BACKEND.enabled&&!key&&!basePdb(curFam)&&member&&!structureFetchFailed[member]){el.innerHTML='<p class="hint" style="padding:12px">Loading structure…</p>';fetchStructure(member,function(){initViewer();});return;}el.innerHTML="";glviewer=$3Dmol.createViewer(el,{backgroundColor:"white"});structMode=preferred;repMode="cartoon";setMode(structMode);}
