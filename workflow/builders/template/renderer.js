...
var domainFiltered=DOMAIN_FAMILIES.slice(),domainNetwork=null,domainNetworkNodes=null,domainNetworkEdges=null,domainSegmentNetwork=null,domainViewer=null,curDomain=null,curDomainSegment=null;
...
function domainMatches(d,query){var members=domainMembers(d.domain_family),text=[d.domain_family,d.n_segments,d.n_proteins,d.n_edges,d.mean_probability,d.mean_lddt,d.mean_aligned_residues,d.top_annotation].concat(members.reduce(function(a,m){return a.concat([m.acc,m.segment_id,m.start,m.end,m.parent_family,m.gene,m.eff,m.tmr,m.pfam,m.ipr,m.pdb,m.pdb_tm,m.afdb,m.afdb_hit,m.afdb_tm,m.novel?"novel":"known"].concat((m.overlap_annotations||[]).map(function(x){return x.source+" "+x.label;})));},[])).join(" ").toLowerCase();return searchNorm(query).split(/\s+/).filter(Boolean).every(function(term){var p=term.indexOf(":");return text.indexOf(p>0?term.slice(p+1):term)>=0;});}
...
function dlDomainEdges(id){var cols=["source","target","evalue","prob","bits","lddt","fident","alnlen","shorter_coverage"],lines=[cols.join(",")];domainEdges(id).forEach(function(e){lines.push(cols.map(function(c){return csvCell(e[c]);}).join(","));});dlText(lines.join("\n")+"\n",id+"_local_foldseek_edges.csv","text/csv");}
...
function dlDomainSegment(){var m=DOMAIN_MEMBERS.find(function(x){return x.segment_id===curDomainSegment;});if(!m)return;var pdb=segmentPdbText(m);if(!pdb){alert("No embedded structure for "+m.acc);return;}dlText(pdb,m.segment_id.replace(":","_")+".pdb");}
function showDomain(id){
 var d=DOMAIN_FAMILIES.find(function(x){return String(x.domain_family)===String(id);}),members=domainMembers(id),edges=domainEdges(id);if(!d)return;curDomain=id;
 document.getElementById("side").innerHTML='<h2>'+esc(id)+(d.top_annotation?' \u00b7 '+esc(d.top_annotation):"")+'</h2><div class="hint">Local Foldseek 3Di+AA family. Nodes below are matched protein segments; edges are local structural alignments.</div><div class="domain-summary"><div class="domain-stat"><b>'+d.n_proteins+'</b>proteins</div><div class="domain-stat"><b>'+d.n_segments+'</b>segments</div><div class="domain-stat"><b>'+fnum(d.mean_lddt,3)+'</b>mean local lDDT</div><div class="domain-stat"><b>'+fnum(d.mean_probability,3)+'</b>mean probability</div><div class="domain-stat"><b>'+fnum(d.mean_aligned_residues,0)+'</b>aligned residues</div><div class="domain-stat"><b>'+d.n_annotated_segments+'</b>annotated segments</div></div><div class="domain-actions"><button onclick="dlDomainFamily(\''+esc(id)+'\')">\u2b07 Segments CSV</button><button onclick="dlDomainEdges(\''+esc(id)+'\')">\u2b07 Foldseek edges CSV</button><button onclick="dlDomainSeqs(\''+esc(id)+'\')">\u2b07 Segment sequences</button></div><h3>Segment similarity network</h3><div id="domainSegmentNet"></div><div id="domainEvidence" class="domain-evidence"><p class="hint">Select a segment to inspect its structure and evidence.</p></div>';
 var snodes=members.map(function(m){var ann=(m.overlap_annotations||[]).map(function(x){return x.label;}).join(", ");return{id:m.segment_id,label:m.acc+"\n"+m.start+"\u2013"+m.end,value:m.length,color:{background:m.parent_family==="singleton"?"#5a9f68":"#4388a1",border:"#355b69"},title:esc(m.segment_id)+(ann?"<br>"+esc(ann):"")+"<br>"+esc(m.parent_family==="singleton"?"singleton":m.parent_family)};});
 var sedges=edges.map(function(e,i){return{id:"dseg-"+i,from:e.source,to:e.target,value:Math.max(.1,Number(e.lddt)||.1),title:"local lDDT "+fnum(e.lddt,3)+"<br>probability "+fnum(e.prob,3)+"<br>aligned "+e.alnlen+" residues<br>identity "+fnum(100*e.fident,1)+"%",color:{color:"#9aadb5"}};});
 ...
 domainSegmentNetwork.on("click",function(p){if(p.nodes.length)showDomainSegment(p.nodes[0]);});if(members.length){domainSegmentNetwork.selectNodes([members[0].segment_id]);showDomainSegment(members[0].segment_id);}
}
function showDomainSegment(segmentId){
 var m=DOMAIN_MEMBERS.find(function(x){return x.segment_id===segmentId;});if(!m)return;curDomainSegment=segmentId;var anns=m.overlap_annotations||[],neighbors=domainEdges(m.domain_family).filter(function(e){return e.source===segmentId||e.target===segmentId;}),rna=Object.keys(m.expression||{}).sort(function(a,b){return Number(m.expression[b])-Number(m.expression[a]);}).slice(0,6);
 var annHtml=anns.length?anns.map(function(a){return'<span class="domain-tag">'+esc(a.source)+': '+esc(a.label)+' ('+a.start+'\u2013'+a.end+')</span>';}).join(""):'<span class="hint">No coordinate-overlapping Pfam/InterPro call.</span>';
 var hitRows=row("Full-length placement",m.parent_family==="singleton"?"singleton":m.parent_family)+row("Gene",m.gene||"\u2013")+row("EffectorP",m.eff||"\u2013")+row("TM helices",m.tmr)+row("Foldseek PDB100",(m.pdb||"\u2013")+(m.pdb_tm!=null?" \u00b7 TM "+fnum(m.pdb_tm,3):""))+row("Foldseek AFDB / Swiss-Prot",(m.afdb||m.afdb_hit||"\u2013")+(m.afdb_tm!=null?" \u00b7 TM "+fnum(m.afdb_tm,3):""))+row("Pocket residues in segment",(m.pocket_residues||[]).length?(m.pocket_residues||[]).join(", "):"\u2013");
 var edgeRows=neighbors.slice(0,12).map(function(e){var other=e.source===segmentId?e.target:e.source;return'<tr><td>'+esc(other)+'</td><td>lDDT '+fnum(e.lddt,3)+' \u00b7 prob '+fnum(e.prob,3)+' \u00b7 '+e.alnlen+' aa</td></tr>';}).join("");
 var rnaRows=rna.length?'<table>'+rna.map(function(k){return row(k,fnum(m.expression[k],2));}).join("")+'</table>':'<p class="hint">RNA-seq data unavailable.</p>';
 document.getElementById("domainEvidence").innerHTML='<h3>'+esc(m.segment_id)+'</h3><div class="domain-actions"><button onclick="openProtein(\''+String(m.acc).replace(/'/g,"\\'")+'\')">Open full protein workbench</button><button onclick="dlDomainSegment()">\u2b07 Segment PDB</button></div><div id="domain3d"></div><div class="hint"><span class="swatch" style="background:#c7cdd0"></span>full protein <span class="swatch" style="background:#00879b"></span>matched domain segment <span class="swatch" style="background:#c83b35"></span>pocket residue in segment</div><h3>Coordinate-overlapping annotation</h3><div class="domain-tags">'+annHtml+'</div><h3>Protein-level evidence</h3><table>'+hitRows+'</table><h3>Local structural neighbours</h3>'+(edgeRows?'<table>'+edgeRows+'</table>':'<p class="hint">No retained edge.</p>')+'<h3>RNA-seq</h3>'+rnaRows;
 renderDomainStructure(m);
}
function renderDomainStructure(m){var el=document.getElementById("domain3d"),pdb=domainPdb(m);if(!el)return;if(!pdb){el.innerHTML='<p class="hint" style="padding:12px">Structure is not embedded in this atlas mode.</p>';return;}domainViewer=$3Dmol.createViewer(el,{backgroundColor:"white"});domainViewer.addModel(pdb,"pdb");domainViewer.setStyle({},{cartoon:{color:"#c7cdd0",opacity:.75}});var res=[];for(var r=m.start;r<=m.end;r++)res.push(r);domainViewer.setStyle({resi:res},{cartoon:{color:"#00879b"},stick:{color:"#00879b",radius:.12}});if((m.pocket_residues||[]).length)domainViewer.setStyle({resi:m.pocket_residues},{stick:{color:"#c83b35",radius:.22}});domainViewer.zoomTo({resi:res});domainViewer.render();}
...
function initViewer(){var el=document.getElementById("v3d");if(!el)return;el.innerHTML="";glviewer=$3Dmol.createViewer(el,{backgroundColor:"white"});var singleton=PAY[curFam]&&PAY[curFam].kind==="singleton";structMode=singleton?"quality":(REFPDB[curFam+"_struct"]?"structcons":"evolcons");repMode="cartoon";setMode(structMode);}
