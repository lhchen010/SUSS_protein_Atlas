...
var domainFiltered=DOMAIN_FAMILIES.slice(),domainNetwork=null,domainNetworkNodes=null,domainNetworkEdges=null,domainSegmentNetwork=null,domainViewer=null,curDomain=null,curDomainSegment=null,domainSuperpose=false;
...
function buildDomainStructurePane(id,query){
 ...
 document.getElementById("dp0").innerHTML='<div class="domain-toolbar"><select id="domainMemberSelect" onchange="showDomainSegment(this.value)">'+opts+'</select><button id="domainSingleBtn" class="on" onclick="setDomainSuperpose(false)">Selected segment</button><button id="domainSuperBtn" onclick="setDomainSuperpose(true)">Superpose family</button><button onclick="domainViewer&&domainViewer.zoomTo()">Fit</button><button onclick="dlDomainSegment()">\u2b07 Segment PDB</button></div><div id="domain3d"></div><div class="hint"><span class="swatch" style="background:#c7cdd0"></span>full protein <span class="swatch" style="background:#00879b"></span>selected domain <span class="swatch" style="background:#c83b35"></span>pocket residues. Superposition uses domain-segment US-align transforms to the D-family hub.</div><h3>Segment similarity network</h3><div id="domainSegmentNet"></div><div id="domainEvidence" class="domain-evidence"></div>';
...
function showDomainSegment(segmentId){
 ...
 var box=document.getElementById("domainEvidence");if(box)box.innerHTML='<h3>'+esc(m.segment_id)+(m.segment_id===domainWorkbench(m.domain_family).hub?' · family hub':"")+'</h3><div class="domain-actions"><button onclick="openProtein(\''+String(m.acc).replace(/'/g,"\\'")+'\')">Open full protein workbench</button></div><div class="domain-tags">'+annHtml+'</div>'+(edgeRows?'<table>'+edgeRows+'</table>':'<p class="hint">No retained local edge.</p>');
...
function setDomainSuperpose(value){domainSuperpose=value;var a=document.getElementById("domainSingleBtn"),b=document.getElementById("domainSuperBtn");if(a)a.className=value?"":"on";if(b)b.className=value?"on":"";var m=DOMAIN_MEMBERS.find(function(x){return x.segment_id===curDomainSegment;});if(m)renderDomainStructure(m);}
function renderDomainStructure(m){var el=document.getElementById("domain3d");if(!el)return;if(domainSuperpose&&BACKEND.enabled){var missing=domainMembers(m.domain_family).some(function(seg){return !domainPdb(seg)&&!structureFetchFailed[seg.acc];});if(missing){el.innerHTML='<p class="hint" style="padding:12px">Loading domain-family structures…</p>';var segments=domainMembers(m.domain_family),pending=segments.length;segments.forEach(function(seg){fetchStructure(seg.acc,function(){pending--;if(!pending)renderDomainStructure(m);});});return;}}domainViewer=$3Dmol.createViewer(el,{backgroundColor:"white"});if(domainSuperpose){var wb=domainWorkbench(m.domain_family),palette=["#00879b","#d06b35","#5a8f4e","#8b65a5","#c39422","#4776a8"];domainMembers(m.domain_family).forEach(function(seg,i){var pdb=segmentPdbText(seg),tr=(wb.transforms||{})[seg.segment_id];if(pdb&&tr){domainViewer.addModel(transformDomainPdb(pdb,tr),"pdb");domainViewer.setStyle({model:i},{cartoon:{color:seg.segment_id===wb.hub?"#d39b2a":palette[i%palette.length],opacity:.78}});}});domainViewer.zoomTo();domainViewer.render();return;}var pdb=domainPdb(m);if(!pdb&&BACKEND.enabled&&!structureFetchFailed[m.acc]){el.innerHTML='<p class="hint" style="padding:12px">Loading structure…</p>';fetchStructure(m.acc,function(text){if(text)renderDomainStructure(m);else el.innerHTML='<p class="hint" style="padding:12px">Structure unavailable.</p>';});return;}if(!pdb){el.innerHTML='<p class="hint" style="padding:12px">Structure unavailable.</p>';return;}domainViewer.addModel(pdb,"pdb");domainViewer.setStyle({},{cartoon:{color:"#c7cdd0",opacity:.55}});var res=[];for(var r=m.start;r<=m.end;r++)res.push(r);domainViewer.setStyle({resi:res},{cartoon:{color:"#00879b",opacity:1}});if((m.pocket_residues||[]).length)domainViewer.setStyle({resi:m.pocket_residues},{stick:{color:"#c83b35",radius:.22}});domainViewer.zoomTo({resi:res});domainViewer.render();}
