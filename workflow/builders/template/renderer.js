...
var domainFiltered=DOMAIN_FAMILIES.slice();
function domainMembers(id){return DOMAIN_MEMBERS.filter(function(m){return String(m.domain_family)===String(id);});}
function domainMatches(d,query){var members=domainMembers(d.domain_family),text=[d.domain_family,d.n_segments,d.n_proteins,d.mean_probability,d.mean_lddt].concat(members.reduce(function(a,m){return a.concat([m.acc,m.segment_id,m.start,m.end]);},[])).join(" ").toLowerCase();return searchNorm(query).split(/\s+/).filter(Boolean).every(function(term){return text.indexOf(term)>=0;});}
function applyDomainSearch(query){domainFiltered=DOMAIN_FAMILIES.filter(function(d){return !query||domainMatches(d,query);});renderDomainRows();var st=document.getElementById("searchstatus");if(st)st.textContent=domainFiltered.length+" domain famil"+(domainFiltered.length===1?"y":"ies");return domainFiltered;}
function renderDomainTable(){var box=document.getElementById("domains");if(!box)return;box.innerHTML='<div class="singleton-head"><div><h2>Domain-aware structural families</h2><div class="hint">Local Foldseek 3Di+AA matches. A protein can occur in more than one D family; D families do not replace full-length F families.</div></div><button onclick="dlDomainFamilies()">Download D families (CSV)</button></div><div class="singleton-table-wrap"><table class="singleton-table"><thead><tr><th>Domain family</th><th>Proteins / segments</th><th>Mean probability</th><th>Mean lDDT</th><th>Mean aligned residues</th><th>Protein segments</th></tr></thead><tbody id="domainRows"></tbody></table></div>';applyDomainSearch((document.getElementById("searchinput")||{}).value||"");}
function renderDomainRows(){var body=document.getElementById("domainRows");if(!body)return;body.innerHTML=domainFiltered.map(function(d){var mem=domainMembers(d.domain_family),buttons=mem.slice(0,12).map(function(m){return'<button class="domain-member" onclick="event.stopPropagation();openProtein(\''+String(m.acc).replace(/'/g,"\\'")+'\')">'+esc(m.acc)+' '+m.start+'\u2013'+m.end+'</button>';}).join("")+(mem.length>12?'<span class="hint">+'+(mem.length-12)+' more</span>':"");return'<tr onclick="showDomain(\''+esc(d.domain_family)+'\')"><td><b>'+esc(d.domain_family)+'</b></td><td>'+d.n_proteins+' / '+d.n_segments+'</td><td>'+fnum(d.mean_probability,3)+'</td><td>'+fnum(d.mean_lddt,3)+'</td><td>'+fnum(d.mean_aligned_residues,1)+'</td><td><div class="domain-members">'+buttons+'</div></td></tr>';}).join("");}
function dlDomainFamilies(){var cols=["domain_family","segment_id","acc","start","end","length"],lines=[cols.join(",")];DOMAIN_MEMBERS.forEach(function(m){lines.push(cols.map(function(c){return csvCell(m[c]);}).join(","));});dlText(lines.join("\n")+"\n","domain_families.csv","text/csv");}
...
function showDomain(id){var d=DOMAIN_FAMILIES.find(function(x){return String(x.domain_family)===String(id);}),members=domainMembers(id);if(!d)return;document.getElementById("side").innerHTML='<h2>'+esc(id)+'</h2><div class="hint">Local structural domain family; independent of whole-protein clustering.</div><table>'+row("Proteins",d.n_proteins)+row("Segments",d.n_segments)+row("Mean Foldseek probability",fnum(d.mean_probability,3))+row("Mean local lDDT",fnum(d.mean_lddt,3))+row("Mean aligned residues",fnum(d.mean_aligned_residues,1))+'</table><h3>Matched segments</h3><div class="domain-members">'+members.map(function(m){return'<button class="domain-member" onclick="openProtein(\''+String(m.acc).replace(/'/g,"\\'")+'\')">'+esc(m.acc)+' \u00b7 '+m.start+'\u2013'+m.end+'</button>';}).join("")+'</div><div class="hint" style="margin-top:8px">Open a protein to inspect its full-length family or singleton evidence, structure, annotation, pocket, and RNA-seq data.</div>';}
...
function setAtlasMode(mode){
 ...
 if(single){renderSingletonTable();document.getElementById("side").innerHTML='<p class="hint">Select a singleton row to inspect its structure and evidence.</p>';}else if(domains){renderDomainTable();document.getElementById("side").innerHTML='<p class="hint">Select a domain family to inspect its matched protein segments.</p>';}else{applyNetworkSearch(input?input.value:"");setTimeout(function(){network.redraw();network.fit();},50);document.getElementById("side").innerHTML='<p class="hint">Click a family node to load data, structure, trees and downloads.</p>';}
}
...
function drawStruct(){
 ...
 if(structMode==="esm"){
  ...
  applyStyle({},{colorscheme:{prop:"b",gradient:"rwb",min:0,max:100}});
  document.getElementById("leg").innerHTML='<span class="swatch" style="background:#2166ac"></span>structurally variable <span class="swatch" style="background:#b2182b"></span>structurally conserved &middot; FoldMason hub-relative local distance agreement';
