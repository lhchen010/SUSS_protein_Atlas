...
function dlStruct(kind){var fam=curFam,d=EXTRA[fam];
 ...
 if(kind==="super"){var mem=PAY[fam].members,sel=[],mdl=1;mem.forEach(function(m){if(selMembers[m]&&PAY[fam].struct[m]){sel.push("MODEL "+(mdl++)+"\n"+PAY[fam].struct[m]+"\nENDMDL");}});if(!sel.length){alert("No members selected for superposition. Tick members on the tree first.");return;}dlText(sel.join("\n"),fam+"_superposed_"+(mdl-1)+"members.pdb");return;}}
...
function dlAllStruct(){var fam=curFam,st=PAY[fam].struct||{},mem=PAY[fam].members,out=[],n=0;mem.forEach(function(m){if(st[m]){out.push("MODEL "+(++n));out.push(st[m].replace(/\n+$/,""));out.push("ENDMDL");}});if(!n){alert("No structures embedded for "+fam);return;}dlText(out.join("\n")+"\n",fam+"_members_"+n+"structs.pdb");}
...
function drawStruct(){
 ...
 if(structMode==="super"){
  ...
  mem.forEach(function(m){if(selMembers[m]&&PAY[curFam].struct[m]){glviewer.addModel(PAY[curFam].struct[m],"pdb");applyStyle({model:ci},{color:cols[ci%cols.length]});ci++;shown++;}});
  glviewer.zoomTo();glviewer.render();
  document.getElementById("leg").innerHTML="Overlay of "+shown+" selected members ("+repMode+"). Tight core = conserved scaffold; splayed loops = variable surface.";
  return;
 }
