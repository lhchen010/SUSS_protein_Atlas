...
function buildDomainStructurePane(id,query){
 ...
 document.getElementById("dp0").innerHTML=
  ...
  '<div class="domain-toolbar"><span class="hint">Evidence:</span><span class="domain-color-modes"><button id="dcm-domain" class="on" onclick="setDomainColorMode(\'domain\')">Domain</button><button id="dcm-pocket" onclick="setDomainColorMode(\'pocket\')">Pocket</button><button id="dcm-esm" onclick="setDomainColorMode(\'esm\')">ESM</button><button id="dcm-cons" onclick="setDomainColorMode(\'cons\')">Structural conservation</button>'+(hasSequenceConservation?'<button id="dcm-seqcons" onclick="setDomainColorMode(\'seqcons\')">Sequence conservation</button>':'')+'</span></div>'+
...
function renderDomainStructure(m){
 ...
 selected.forEach(function(seg){
  ...
  var values=domainColorMode==="esm"?(seg.esm_values||{}):(domainColorMode==="cons"&&seg.segment_id===wb.hub?(wb.hub_structural_conservation||{}):(domainColorMode==="seqcons"?((wb.sequence_conservation||{})[seg.segment_id]||{}):{}));
  ...
  applyDomainStyle({model:modelIndex},{color:"#c7cdd0",opacity:domainRepMode==="surface"?.18:.22});
...
function buildDomainSequencePane(id){var wb=domainWorkbench(id),identity=wb.sequence_identity_matrix_svg?'<h3>Domain sequence identity heatmap</h3><div class="hint">Pairwise amino-acid identity is measured only across FoldMason-aligned domain residues, so unrelated regions of the complete parent proteins do not inflate the result.</div><img src="'+wb.sequence_identity_matrix_svg+'">':'<p class="hint">Domain sequence-identity matrix was not available.</p>';document.getElementById("dp3").innerHTML=identity+'<div class="hint">Local Foldseek structure links define the D family. MAFFT is restricted to BLAST-supported sequence subgroups; FoldMason covers the complete structural family.</div>'+domainMsaBlock("MAFFT sequence MSA",wb.sequence_msa||{})+domainMsaBlock("FoldMason structural MSA · amino acids",wb.structural_msa||{})+domainMsaBlock("FoldMason structural MSA · 3Di",wb.three_di_msa||{})+'<div class="domain-actions"><button onclick="dlDomainSeqs(\''+esc(id)+'\')">All domain sequences</button><button onclick="dlDomainParentSeqs(\''+esc(id)+'\')">All parent sequences</button><button onclick="dlDomainMsa(\''+esc(id)+'\')">MAFFT MSA</button><button onclick="dlDomainAlignment(\''+esc(id)+'\',\'aa\')">FoldMason AA MSA</button><button onclick="dlDomainAlignment(\''+esc(id)+'\',\'3di\')">FoldMason 3Di MSA</button></div>';}
function buildDomainConservationPane(id){var wb=domainWorkbench(id),sequenceConservation=wb.sequence_conservation||{},rows=domainMembers(id).map(function(m){var p2=m.p2rank||{},fp=m.fpocket||{},esm=Object.keys(m.esm_values||{}).length,seq=Object.keys(sequenceConservation[m.segment_id]||{}).length;return'<tr><td><b>'+esc(m.segment_id)+'</b></td><td>'+((p2.domain_lining_residues||[]).join(", ")||"none")+'</td><td>'+fnum(p2.top_score,3)+' / '+fnum(p2.top_probability,3)+'</td><td>'+((fp.domain_lining_residues||[]).join(", ")||"none")+'</td><td>'+fnum(fp.top_score,3)+'</td><td>'+esm+'</td><td>'+seq+'</td></tr>';}).join(""),sequenceSegments=domainMembers(id).filter(function(m){return Object.keys(sequenceConservation[m.segment_id]||{}).length;}).length;document.getElementById("dp4").innerHTML='<h3>Structural and sequence conservation</h3><div class="hint">FoldMason structural conservation is calculated across the complete structural family. Rate4Site sequence conservation is calculated only inside eligible BLAST-supported MAFFT subgroups, matching the Full-family scientific rule. P2Rank and fpocket are calculated for every complete parent protein. ESM is representative-level by default.</div><div class="domain-summary"><div class="domain-stat"><b>'+Object.keys(wb.hub_structural_conservation||{}).length+'</b>hub residues with FoldMason lDDT</div><div class="domain-stat"><b>'+sequenceSegments+'</b>segments with Rate4Site scores</div><div class="domain-stat"><b>'+domainMembers(id).filter(function(m){return(m.pocket_residues||[]).length;}).length+'</b>segments overlapping a pocket</div><div class="domain-stat"><b>'+domainMembers(id).filter(function(m){return Object.keys(m.esm_values||{}).length;}).length+'</b>segments with ESM values</div></div><div class="domain-edge-table"><table><thead><tr><th>Segment</th><th>P2Rank domain residues</th><th>score / probability</th><th>fpocket domain residues</th><th>score</th><th>ESM sites</th><th>Rate4Site sites</th></tr></thead><tbody>'+rows+'</tbody></table></div>';}
...
var curFam=null,glviewer=null,structMode="structcons",repMode="cartoon",viewerBackground="white",selMembers={},curTree=null,pockMethod="fpocket";
...
function tab(i){var tabs=document.querySelectorAll(".tab");for(var k=0;k<tabs.length;k++)tabs[k].className="tab"+(tabs[k].getAttribute("onclick").indexOf("tab("+i+")")>=0?" on":"");
  ...
  if(i===2&&!document.getElementById("p2").innerHTML){var ex2=EXTRA[curFam]||{};var h2='<h3>Structural similarity (Foldseek TM)</h3>'+(a.tm_svg?'<img src="'+a.tm_svg+'">':'<p class="hint">Foldseek TM matrix was not available for this family.</p>')+dlbtn();if(a.tmus_svg){var cr=(ex2.tm_cons_r!=null)?(' \\u00b7 Foldseek\\u2194US-align r='+fnum(ex2.tm_cons_r,3)+', max\\u0394='+fnum(ex2.tm_cons_maxdiff,2)+', '+(ex2.tm_disagree||0)+' pair(s) disagree >0.1'):'';h2+='<h3 style="margin-top:14px">Structural similarity (US-align TM \\u2014 independent algorithm)</h3><div class="hint" style="margin:2px 0 6px">Foldseek builds the families; US-align (TM-align successor) recomputes true TM within the family as an algorithm-independent cross-check.'+cr+'</div><img src="'+a.tmus_svg+'">';}else{h2+='<h3 style="margin-top:14px">Structural similarity (US-align TM)</h3><p class="hint">Independent US-align matrix was not available.</p>';}document.getElementById("p2").innerHTML=h2;}
