...
var atlasMode="clusters",lastFullMode="clusters",lastDomainMode="domains",singletonSort="acc",singletonSortDir=1,singletonPage=0,singletonPageSize=50,singletonFiltered=SINGLETONS.slice();
...
function buildDomainConservationPane(id){var wb=domainWorkbench(id),members=domainMembers(id),sequenceConservation=wb.sequence_conservation||{},rows=members.map(function(m){var p2=m.p2rank||{},fp=m.fpocket||{},esm=Object.keys(m.esm_values||{}).length,seq=Object.keys(sequenceConservation[m.segment_id]||{}).length;return'<tr><td><b>'+esc(m.segment_id)+'</b></td><td>'+((p2.domain_lining_residues||[]).join(", ")||"none")+'</td><td>'+fnum(p2.top_score,3)+' / '+fnum(p2.top_probability,3)+'</td><td>'+((fp.domain_lining_residues||[]).join(", ")||"none")+'</td><td>'+fnum(fp.top_score,3)+'</td><td>'+esm+'</td><td>'+seq+'</td></tr>';}).join(""),structuralSegments=members.filter(function(m){return Object.keys(domainStructuralValues(m,wb)).length;}).length,sequenceSegments=members.filter(function(m){return Object.keys(sequenceConservation[m.segment_id]||{}).length;}).length;document.getElementById("dp4").innerHTML='<h3>Structural and sequence conservation</h3><div class="hint">FoldMason structural conservation is calculated across the complete structural family and mapped through the structural MSA onto every aligned segment. Rate4Site sequence conservation is calculated only inside eligible BLAST-supported MAFFT subgroups, matching the Full-family scientific rule. P2Rank and fpocket are calculated for every complete parent protein. ESM is representative-level by default.</div><div class="domain-summary"><div class="domain-stat"><b>'+structuralSegments+'</b>segments with FoldMason lDDT</div><div class="domain-stat"><b>'+sequenceSegments+'</b>segments with Rate4Site scores</div><div class="domain-stat"><b>'+members.filter(function(m){return(m.pocket_residues||[]).length;}).length+'</b>segments overlapping a pocket</div><div class="domain-stat"><b>'+members.filter(function(m){return Object.keys(m.esm_values||{}).length;}).length+'</b>segments with ESM values</div></div><div class="domain-edge-table"><table><thead><tr><th>Segment</th><th>P2Rank domain residues</th><th>score / probability</th><th>fpocket domain residues</th><th>score</th><th>ESM sites</th><th>Rate4Site sites</th></tr></thead><tbody>'+rows+'</tbody></table></div>';}
...
var clearSearchButton=document.getElementById("clearsearch");if(clearSearchButton)clearSearchButton.addEventListener("click",clearAtlasSearch);document.getElementById("modeclusters").textContent="Families ("+NET.nodes.length+")";document.getElementById("modesingletons").textContent="Singletons ("+SINGLETONS.length+")";document.getElementById("modedomains").textContent="Families ("+DOMAIN_FAMILIES.length+")";document.getElementById("modeunassigned").textContent="No retained match ("+DOMAIN_UNASSIGNED.length+")";if(ANALYSIS_SCOPE==="full"){document.getElementById("scopedomain").style.display="none";document.getElementById("domainModeTabs").style.display="none";}if(ANALYSIS_SCOPE==="domain"){document.getElementById("scopefull").style.display="none";document.getElementById("fullModeTabs").style.display="none";setAtlasMode("domains");}else setAtlasMode("clusters");
