...
var domainFiltered=DOMAIN_FAMILIES.slice(),domainNetwork=null,domainNetworkNodes=null,domainNetworkEdges=null,domainSegmentNetwork=null,domainViewer=null,curDomain=null,curDomainSegment=null,domainSuperpose=false,domainParentContext=false,domainSelected={};
...
function showDomain(id){
 ...
 var support=d.top_annotation?d.top_annotation_count+"/"+d.n_segments+" segments":"no coordinate-overlapping annotation";
 document.getElementById("side").innerHTML='<h2>'+esc(id)+(d.top_annotation?' \u00b7 '+esc(d.top_annotation):"")+'</h2><div class="domain-support"><strong>'+esc(support)+'</strong>. A D family is defined by retained local Foldseek 3Di+AA links; the label describes only coordinate-overlapping evidence.</div><div class="domain-summary"><div class="domain-stat"><b>'+d.n_proteins+'</b>proteins</div><div class="domain-stat"><b>'+d.n_segments+'</b>segments</div><div class="domain-stat"><b>'+fnum(d.mean_lddt,3)+'</b>retained-edge lDDT</div><div class="domain-stat"><b>'+fnum(d.mean_alntm,3)+'</b>alignment TM</div><div class="domain-stat"><b>'+fnum(d.mean_aligned_residues,0)+'</b>aligned residues</div><div class="domain-stat"><b>'+d.n_annotated_segments+'</b>annotated segments</div></div><div class="tabs"><div class="tab on" onclick="domainTab(0)">Structure + Network</div><div class="tab" onclick="domainTab(1)">Struct sim</div><div class="tab" onclick="domainTab(2)">Sequence + MSA</div><div class="tab" onclick="domainTab(3)">RNA-seq</div><div class="tab" onclick="domainTab(4)">Annotation</div></div><div id="dp0" class="pane on"></div><div id="dp1" class="pane"></div><div id="dp2" class="pane"></div><div id="dp3" class="pane"></div><div id="dp4" class="pane"></div>';
 buildDomainStructurePane(id,query);
}
function domainTab(i){for(var k=0;k<5;k++){var pane=document.getElementById("dp"+k);if(pane)pane.className="pane"+(k===i?" on":"");}document.querySelectorAll("#side .tab").forEach(function(tab,k){tab.className="tab"+(k===i?" on":"");});if(i===1&&!document.getElementById("dp1").innerHTML)buildDomainSimilarityPane(curDomain);if(i===2&&!document.getElementById("dp2").innerHTML)buildDomainSequencePane(curDomain);if(i===3&&!document.getElementById("dp3").innerHTML)buildDomainRnaPane(curDomain);if(i===4&&!document.getElementById("dp4").innerHTML)buildDomainAnnotationPane(curDomain);}
function buildDomainStructurePane(id,query){
 var members=domainMembers(id),edges=domainEdges(id),wb=domainWorkbench(id),matched=domainMatchingMembers(id,query||""),initial=(matched[0]||members.find(function(m){return m.segment_id===wb.hub;})||members[0]),opts=members.map(function(m){return'<option value="'+esc(m.segment_id)+'">'+esc(m.acc+" · "+m.start+"–"+m.end)+(m.segment_id===wb.hub?" · hub":"")+'</option>';}).join("");
 domainSelected={};if(initial)domainSelected[initial.segment_id]=true;if(wb.hub)domainSelected[wb.hub]=true;if(Object.keys(domainSelected).length<2&&members.length>1){var comparison=members.find(function(m){return !domainSelected[m.segment_id];});if(comparison)domainSelected[comparison.segment_id]=true;}domainParentContext=false;
 var choices=members.map(function(m){var plen=(domainSeq(m)||"").length||Number(m.parent_length)||Number(m.end)||1,left=Math.max(0,Math.min(100,(Number(m.start)-1)/plen*100)),width=Math.max(1,Math.min(100-left,(Number(m.end)-Number(m.start)+1)/plen*100)),color=domainSegmentColor(m);return'<div class="domain-choice" data-domain-choice="'+esc(m.segment_id)+'"><input type="checkbox" id="dc-'+esc(m.segment_id).replace(/[^A-Za-z0-9_-]/g,"-")+'" data-domain-check="'+esc(m.segment_id)+'" onchange="toggleDomainSelection(\''+String(m.segment_id).replace(/'/g,"\\'")+'\',this.checked)"><label onclick="showDomainSegment(\''+String(m.segment_id).replace(/'/g,"\\'")+'\')" title="'+esc(m.segment_id)+'"><span class="swatch" style="background:'+color+'"></span>'+esc(m.acc)+(m.segment_id===wb.hub?" · hub":"")+'</label><div class="domain-architecture" title="'+m.start+"–"+m.end+" / "+plen+" aa"+'"><span style="left:'+left+'%;width:'+width+'%;background:'+color+'"></span></div></div>';}).join("");
 document.getElementById("dp0").innerHTML='<div class="domain-toolbar"><select id="domainMemberSelect" onchange="showDomainSegment(this.value)">'+opts+'</select><button id="domainSingleBtn" class="on" onclick="setDomainSuperpose(false)">Selected segment</button><button id="domainSuperBtn" onclick="setDomainSuperpose(true)">Superpose selected</button><button id="domainContextBtn" class="on" onclick="toggleDomainParentContext()" title="Show the focused full-length parent behind the aligned domains">Parent context</button><button onclick="domainViewer&&domainViewer.zoomTo()">Fit</button><button onclick="dlDomainSegment()">\u2b07 Segment PDB</button><button onclick="dlDomainSuperposition()">\u2b07 Selected superposition</button></div><div class="domain-selection"><div class="domain-selection-head"><b>Select domains to superpose</b><span><button onclick="selectDomainMembers(\'all\')">All</button><button onclick="selectDomainMembers(\'none\')">None</button><button onclick="selectDomainMembers(\'neighbors\')">Neighbors</button></span></div><div class="domain-selection-list">'+choices+'</div></div><div id="domain3d"></div><div id="domainPosition" class="domain-position"></div><div id="domainLegend" class="domain-superpose-legend"></div><div class="hint">Selected segment shows the full parent in gray and its matched domain in color. Superpose selected uses domain-segment US-align transforms to the gold family hub.</div><h3>Segment similarity network</h3><div id="domainSegmentNet"></div><div id="domainEvidence" class="domain-evidence"></div>';
 ...
 domainSegmentNetwork.on("click",function(p){if(p.nodes.length)showDomainSegment(p.nodes[0]);});domainSegmentNetwork.once("stabilizationIterationsDone",function(){domainSegmentNetwork.stopSimulation();});if(initial){document.getElementById("domainMemberSelect").value=initial.segment_id;domainSegmentNetwork.selectNodes([initial.segment_id]);showDomainSegment(initial.segment_id);}
}
function showDomainSegment(segmentId){
 var m=DOMAIN_MEMBERS.find(function(x){return x.segment_id===segmentId;});if(!m)return;curDomainSegment=segmentId;var select=document.getElementById("domainMemberSelect");if(select)select.value=segmentId;if(domainSegmentNetwork)domainSegmentNetwork.selectNodes([segmentId]);var anns=m.overlap_annotations||[],neighbors=domainEdges(m.domain_family).filter(function(e){return e.source===segmentId||e.target===segmentId;});
 ...
 var box=document.getElementById("domainEvidence");if(box)box.innerHTML='<h3>'+esc(m.segment_id)+(m.segment_id===domainWorkbench(m.domain_family).hub?' · family hub':"")+'</h3><div class="domain-actions"><button onclick="openProtein(\''+String(m.acc).replace(/'/g,"\\'")+'\')">Open full protein workbench</button><button onclick="toggleDomainSelection(\''+String(m.segment_id).replace(/'/g,"\\'")+'\',true)">Add to superposition</button></div><div class="domain-tags">'+annHtml+'</div>'+(edgeRows?'<table>'+edgeRows+'</table>':'<p class="hint">No retained local edge.</p>');
 updateDomainSelectionUi();
...
function updateDomainSelectionUi(){if(!curDomain)return;var selected=domainSelectedMembers(curDomain);document.querySelectorAll("[data-domain-check]").forEach(function(box){box.checked=!!domainSelected[box.getAttribute("data-domain-check")];});var superBtn=document.getElementById("domainSuperBtn");if(superBtn){superBtn.textContent="Superpose selected ("+selected.length+")";superBtn.disabled=selected.length<2;}var contextBtn=document.getElementById("domainContextBtn");if(contextBtn)contextBtn.className=domainParentContext?"on":"";var legend=document.getElementById("domainLegend");if(legend)legend.innerHTML=(domainSuperpose?selected:[DOMAIN_MEMBERS.find(function(x){return x.segment_id===curDomainSegment;})]).filter(Boolean).map(function(m){return'<span><i class="swatch" style="background:'+domainSegmentColor(m)+'"></i>'+esc(m.acc+" "+m.start+"–"+m.end)+(m.segment_id===domainWorkbench(m.domain_family).hub?" · hub":"")+'</span>';}).join("")+(domainSuperpose&&domainParentContext?'<span><i class="swatch" style="background:#c7cdd0"></i>focused full parent</span>':'');}
function toggleDomainSelection(segmentId,checked){domainSelected[segmentId]=!!checked;updateDomainSelectionUi();if(domainSuperpose){var m=DOMAIN_MEMBERS.find(function(x){return x.segment_id===curDomainSegment;});if(domainSelectedMembers(curDomain).length<2)setDomainSuperpose(false);else if(m)renderDomainStructure(m);}}
function selectDomainMembers(mode){var members=domainMembers(curDomain),chosen={};if(mode==="all")members.forEach(function(m){chosen[m.segment_id]=true;});else if(mode==="neighbors"){if(curDomainSegment)chosen[curDomainSegment]=true;domainEdges(curDomain).forEach(function(e){if(e.source===curDomainSegment)chosen[e.target]=true;if(e.target===curDomainSegment)chosen[e.source]=true;});}domainSelected=chosen;updateDomainSelectionUi();if(domainSuperpose){var m=DOMAIN_MEMBERS.find(function(x){return x.segment_id===curDomainSegment;});if(domainSelectedMembers(curDomain).length<2)setDomainSuperpose(false);else if(m)renderDomainStructure(m);}}
function toggleDomainParentContext(){domainParentContext=!domainParentContext;updateDomainSelectionUi();var m=DOMAIN_MEMBERS.find(function(x){return x.segment_id===curDomainSegment;});if(domainSuperpose&&m)renderDomainStructure(m);}
function setDomainSuperpose(value){if(value&&domainSelectedMembers(curDomain).length<2){alert("Select at least two domain members to superpose.");return;}domainSuperpose=value;var a=document.getElementById("domainSingleBtn"),b=document.getElementById("domainSuperBtn");if(a)a.className=value?"":"on";if(b)b.className=value?"on":"";updateDomainSelectionUi();var m=DOMAIN_MEMBERS.find(function(x){return x.segment_id===curDomainSegment;});if(m)renderDomainStructure(m);}
function dlDomainSuperposition(){var selected=domainSelectedMembers(curDomain),wb=domainWorkbench(curDomain);if(selected.length<2){alert("Select at least two domain members.");return;}var missing=selected.filter(function(seg){return !domainPdb(seg)&&BACKEND.enabled&&!structureFetchFailed[seg.acc];});if(missing.length){var pending=missing.length;missing.forEach(function(seg){fetchStructure(seg.acc,function(){pending--;if(!pending)dlDomainSuperposition();});});return;}var models=[];selected.forEach(function(seg,i){var pdb=segmentPdbText(seg),tr=(wb.transforms||{})[seg.segment_id];if(pdb&&tr)models.push("MODEL     "+String(i+1).padStart(4," ")+"\nREMARK 900 "+seg.segment_id+"\n"+transformDomainPdb(pdb,tr).replace(/\nEND\s*\n?$/,"\n")+"ENDMDL");});if(models.length<2){alert("Aligned structures were unavailable for the selected members.");return;}dlText(models.join("\n")+"\nEND\n",curDomain+"_selected_domains_superposed.pdb");}
function renderDomainStructure(m){var el=document.getElementById("domain3d");if(!el)return;var selected=domainSuperpose?domainSelectedMembers(m.domain_family):[m];if(domainSuperpose&&BACKEND.enabled){var missing=selected.filter(function(seg){return !domainPdb(seg)&&!structureFetchFailed[seg.acc];});if(domainParentContext&&!domainPdb(m)&&!structureFetchFailed[m.acc]&&!missing.some(function(seg){return seg.acc===m.acc;}))missing.push(m);if(missing.length){el.innerHTML='<p class="hint" style="padding:12px">Loading selected structures…</p>';var pending=missing.length;missing.forEach(function(seg){fetchStructure(seg.acc,function(){pending--;if(!pending)renderDomainStructure(m);});});return;}}domainViewer=$3Dmol.createViewer(el,{backgroundColor:"white"});if(domainSuperpose){var wb=domainWorkbench(m.domain_family),modelIndex=0,focusTr=(wb.transforms||{})[m.segment_id],parent=domainPdb(m);if(domainParentContext&&parent&&focusTr){domainViewer.addModel(transformDomainPdb(parent,focusTr),"pdb");domainViewer.setStyle({model:modelIndex},{cartoon:{color:"#c7cdd0",opacity:.25}});modelIndex++;}selected.forEach(function(seg){var pdb=segmentPdbText(seg),tr=(wb.transforms||{})[seg.segment_id];if(pdb&&tr){domainViewer.addModel(transformDomainPdb(pdb,tr),"pdb");domainViewer.setStyle({model:modelIndex},{cartoon:{color:domainSegmentColor(seg),opacity:.88}});modelIndex++;}});domainViewer.zoomTo();domainViewer.render();updateDomainSelectionUi();return;}var pdb=domainPdb(m);if(!pdb&&BACKEND.enabled&&!structureFetchFailed[m.acc]){el.innerHTML='<p class="hint" style="padding:12px">Loading structure…</p>';fetchStructure(m.acc,function(text){if(text)renderDomainStructure(m);else el.innerHTML='<p class="hint" style="padding:12px">Structure unavailable.</p>';});return;}if(!pdb){el.innerHTML='<p class="hint" style="padding:12px">Structure unavailable.</p>';return;}domainViewer.addModel(pdb,"pdb");domainViewer.setStyle({},{cartoon:{color:"#c7cdd0",opacity:.55}});var res=[];for(var r=m.start;r<=m.end;r++)res.push(r);domainViewer.setStyle({resi:res},{cartoon:{color:domainSegmentColor(m),opacity:1}});if((m.pocket_residues||[]).length)domainViewer.setStyle({resi:m.pocket_residues},{stick:{color:"#c83b35",radius:.22}});domainViewer.zoomTo({resi:res});domainViewer.render();updateDomainSelectionUi();}
function buildDomainSimilarityPane(id){var d=DOMAIN_FAMILIES.find(function(x){return x.domain_family===id;})||{},rows=domainEdges(id).slice().sort(function(a,b){return Number(b.lddt)-Number(a.lddt);}).map(function(e){return'<tr><td>'+esc(e.source)+'</td><td>'+esc(e.target)+'</td><td>'+fnum(e.lddt,3)+'</td><td>'+fnum(e.alntmscore,3)+'</td><td>'+fnum(e.prob,3)+'</td><td>'+e.alnlen+'</td><td>'+fnum(e.qcov,2)+' / '+fnum(e.tcov,2)+'</td></tr>';}).join("");document.getElementById("dp1").innerHTML='<h3>Retained local Foldseek links</h3><div class="hint">lDDT and alignment TM describe local structural agreement. Coverage is query / target alignment coverage and may be low by design for domain-aware clustering.</div><div class="domain-edge-table"><table><thead><tr><th>Segment A</th><th>Segment B</th><th>lDDT</th><th>alnTM</th><th>Prob</th><th>Aligned</th><th>Coverage</th></tr></thead><tbody>'+rows+'</tbody></table></div><div class="domain-actions"><button onclick="dlDomainEdges(\''+esc(id)+'\')">\u2b07 Foldseek edges CSV</button></div>';}
function buildDomainSequencePane(id){var wb=domainWorkbench(id),records=wb.sequence_msa||{},msa=Object.keys(records).length?sequenceMsaText(records,Object.keys(records)):"Domain sequence MSA unavailable.",tree=wb.sequence_tree_svg?'<img src="'+wb.sequence_tree_svg+'">':'<p class="hint">Sequence tree unavailable.</p>';document.getElementById("dp2").innerHTML='<h3>MAFFT domain-segment sequence MSA</h3><div class="hint">MAFFT aligns only the coordinates represented by this D family. It does not define the family; local Foldseek structure links do.</div><pre class="sequence-view">'+esc(msa)+'</pre><div class="domain-actions"><button onclick="dlDomainSeqs(\''+esc(id)+'\')">\u2b07 Segment sequences</button><button onclick="dlDomainMsa(\''+esc(id)+'\')">\u2b07 Sequence MSA</button></div><h3>Sequence relationship tree</h3>'+tree;}
function buildDomainRnaPane(id){var members=domainMembers(id),conditions={};members.forEach(function(m){Object.keys(m.expression||{}).forEach(function(k){conditions[k]=true;});});var keys=Object.keys(conditions),head='<tr><th>Segment</th>'+keys.map(function(k){return'<th>'+esc(k)+'</th>';}).join("")+'</tr>',rows=members.map(function(m){return'<tr><td>'+esc(m.segment_id)+'</td>'+keys.map(function(k){return'<td>'+fnum((m.expression||{})[k],2)+'</td>';}).join("")+'</tr>';}).join("");document.getElementById("dp3").innerHTML=keys.length?'<h3>RNA-seq by parent protein</h3><div class="domain-edge-table"><table><thead>'+head+'</thead><tbody>'+rows+'</tbody></table></div>':'<p class="hint">RNA-seq data were not available for this run.</p>';}
function buildDomainAnnotationPane(id){var rows=domainMembers(id).map(function(m){var overlap=(m.overlap_annotations||[]).map(function(a){return a.source+": "+a.label+" ("+a.start+"–"+a.end+")";}).join("; ")||"none",protein=[m.gene,m.eff,m.pfam,m.ipr,m.pdb,m.afdb||m.afdb_hit].filter(Boolean).join("; ")||"none";return'<tr><td><b>'+esc(m.segment_id)+'</b><br><button onclick="openProtein(\''+String(m.acc).replace(/'/g,"\\'")+'\')">Open protein</button></td><td>'+esc(overlap)+'</td><td>'+esc(protein)+'</td><td>'+((m.pocket_residues||[]).join(", ")||"none")+'</td></tr>';}).join("");document.getElementById("dp4").innerHTML='<h3>Coordinate-aware annotation</h3><div class="hint">The domain label and domain search use only Pfam/InterPro calls that overlap the segment coordinates. Protein-level evidence is shown separately.</div><div class="domain-edge-table"><table><thead><tr><th>Segment</th><th>Overlapping domain call</th><th>Parent-protein evidence</th><th>Pocket residues</th></tr></thead><tbody>'+rows+'</tbody></table></div><div class="domain-actions"><button onclick="dlDomainFamily(\''+esc(id)+'\')">\u2b07 Segments CSV</button></div>';}
...
function applyActiveSearch(query){return atlasMode==="singletons"?applySingletonSearch(query):(atlasMode==="domains"?applyDomainSearch(query):applyNetworkSearch(query));}
...
function setAtlasMode(mode){
 atlasMode=mode;var single=mode==="singletons",domains=mode==="domains";document.getElementById("net").classList.toggle("mode-hidden",single||domains);document.getElementById("singletons").classList.toggle("mode-hidden",!single);document.getElementById("domains").classList.toggle("mode-hidden",!domains);document.getElementById("modeclusters").className=(!single&&!domains)?"on":"";document.getElementById("modedomains").className=domains?"on":"";document.getElementById("modesingletons").className=single?"on":"";
 var input=document.getElementById("searchinput");if(input)input.placeholder=single?"Search singleton gene, annotation, Foldseek hit":(domains?"Search D family, protein, or segment":"Search full-length families");
 if(single){renderSingletonTable();document.getElementById("side").innerHTML='<p class="hint">Select a singleton row to inspect its structure and evidence.</p>';}else if(domains){renderDomainTable();document.getElementById("side").innerHTML='<p class="hint">The overview network connects D families when retained local Foldseek hits bridge their segment communities. Select a node for the segment-level network, structure, annotation, and evidence.</p>';setTimeout(function(){if(domainNetwork){domainNetwork.redraw();domainNetwork.fit();}},50);}else{applyNetworkSearch(input?input.value:"");setTimeout(function(){network.redraw();network.fit();},50);document.getElementById("side").innerHTML='<p class="hint">Click a family node to load data, structure, trees and downloads.</p>';}
}
...
var clearSearchButton=document.getElementById("clearsearch");if(clearSearchButton)clearSearchButton.addEventListener("click",clearAtlasSearch);document.getElementById("modesingletons").textContent="Singletons ("+SINGLETONS.length+")";document.getElementById("modedomains").textContent="Domain families ("+DOMAIN_FAMILIES.length+")";applyNetworkSearch("");
var curFam=null,glviewer=null,structMode="structcons",repMode="cartoon",selMembers={},curTree=null,pockMethod="fpocket";
...
function dlStruct(kind){var fam=curFam,d=EXTRA[fam];
 ...
 if(kind==="structcons"){if(!REFPDB[fam+"_struct"]){alert("Structural conservation was not available for "+fam);return;}dlText(REFPDB[fam+"_struct"],fam+"_structural_conservation.pdb");return;}
 if(kind==="evolcons"){if(!REFPDB[fam+"_cons"]){alert("Evolutionary conservation was not applicable or unavailable for "+fam);return;}dlText(REFPDB[fam+"_cons"],fam+"_evolutionary_conservation.pdb");return;}
...
function buildStructPane(id){
 ...
 var hasStructural=!!REFPDB[id+"_struct"],hasEvolutionary=!!REFPDB[id+"_cons"];
 var h='<div>'+(hasStructural?'<button id="bstructcons" class="on" onclick="setMode(\'structcons\')">Structural conservation</button>':"")+
...
function initViewer(){var el=document.getElementById("v3d");if(!el)return;var singleton=PAY[curFam]&&PAY[curFam].kind==="singleton",member=(PAY[curFam].members||[])[0];if(BACKEND.enabled&&singleton&&!basePdb(curFam)&&member&&!structureFetchFailed[member]){el.innerHTML='<p class="hint" style="padding:12px">Loading structure…</p>';fetchStructure(member,function(){initViewer();});return;}el.innerHTML="";glviewer=$3Dmol.createViewer(el,{backgroundColor:"white"});structMode=singleton?"quality":(REFPDB[curFam+"_struct"]?"structcons":"evolcons");repMode="cartoon";setMode(structMode);}
...
function drawStruct(){
 ...
 if(structMode==="esm"){
  ...
  document.getElementById("leg").innerHTML='<span class="swatch" style="background:#2166ac"></span>evolutionarily variable <span class="swatch" style="background:#b2182b"></span>evolutionarily conserved &middot; Rate4Site on sequence-homologous subgroup '+fnum(d.cons_min,1)+"\u2026"+fnum(d.cons_max,1);
 } else {
